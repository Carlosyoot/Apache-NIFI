package intelipost.nifi.processors.TokenGenerator;
...
@CapabilityDescription(
    "Executa o processo de geração de um JSON Web Token (JWT), que inclui uma expiração definida " +
    "e propriedades adicionais. Essas propriedades são atribuídas ao JWT como claims, por exemplo, " +
    "um claim com 'email' (valor), que é inserido diretamente no objeto token. O JWT é assinado com uma chave privada " +
    "de um arquivo .pem, fornecida por outro processador do mesmo grupo, chamado 'SecretManager'.\n\n" +
    
    "See also: 'SecretManager' - Processador responsável por fornecer a chave privada necessária para a criação do JWT."
)
@ReadsAttributes({
    @ReadsAttribute(attribute = "jwt.*", description = "Atributos opcionais usados como claims no token JWT")
})
@WritesAttributes({@WritesAttribute(attribute="", description="")})
public class TokenGenerator extends AbstractProcessor {
    ...
    try {
        ...
        ZonedDateTime expirationBrasilia = expiration.toInstant().atZone(ZoneId.of("America/Sao_Paulo"));
        ...
        atributosToken.put("Token.expiration", expirationBrasilia.format(formatter));
